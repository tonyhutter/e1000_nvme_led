...
#define STATLIST_ENTRY(_name) {__ATTR(_name, 0660, craye1k_show, craye1k_store), &stats._name}
...
/* The individual statlist_attrs[] entries get initialized in craye1k_create_sysfs */
	...
	if (msg->msgid != craye1k->tx_msg_id) {
		...
		if (print_errors) {
			dev_warn_ratelimited(craye1k->dev,
				"%s received msgid %d doesn't match expected msgid %d",
...
static int craye1k_do_message(void)
{
	...
	if (rc == 0) {
		rc2 = wait_for_completion_killable_timeout(&craye1k->read_complete,
